...
Для скачивания бинарных сборок под MacOs, Ubuntu x32 или Ubuntu x64, перейдите по [ссылке](https://www.google.com).
...
Примечание: для запуска клиента из любой директории, привяжите его к одной из 
...
# Аутентификация
Прежде чем приступить к работе с Containerum CLI, нужно указать Ваш токен (TOKEN). Токен можно найти на containerum.io/profile.
...
На данный момент пользователь может выбрать <em>Namespace</em>, в котором будет работать.

Пространство пользователя <em>Namespace</em> задано по умолчанию как default. Если у Вас несколько <em>Namespace</em>, то можно явно указать <em>Namespace</em>, выбрав его из имеющихся.

$ chkit config --set-default-namespace NAMESPACE

Пример

Вывод списка всех <em>Namespace</em> пользователя:
...
Сontainerum CLI предоставляет доступ к четырем типам объектов: <em>Namespace</em>, <em>Deployment</em>, <em>Pod</em>, <em>Service</em>.

С помощью Containerum CLI доступно управление тремя типами объектов: <em>Deployment</em>, <em>Pod</em>, <em>Service</em>, переключаясь между <em>Namespace</em.

<em>Namespace</em> - выделенный ресурс, объединяющий объекты пользователя или группы пользователей в единое пространство, на которое выделяется объем памяти и ресурс CPU.

<em>Deployment</em> - контроллер управления одним или несколькими контейнерами, объединенными в <em>Pods</em>.

<em>Pod</em> - группа из одного или нескольких контейнеров. <em>Pod</em> использует выделенный объем памяти и ресурс CPU, указанный в <em>Namespace</em.

<em>Service</em> -объект, обеспечивающий общий доступ к <em>Pod</em>.
...
Примечание: команда вызывается без аргументов. При вызове команды в диалоговом режиме вводятся e-mail и пароль.
...
Создать JSON файл возможно с помощью шаблонов.
...
Примечание: на данным момент доступно изменение параметра image (образа приложения).
