...
var (
	_ animation.FrameSource = &Silly{}
)

type dumb struct{}

func (*dumb) Next() bool    { return true }
func (*dumb) Frame() string { return "" }

type Silly struct {
	prefixSource animation.FrameSource
}

func NewSilly() *Silly {
	return &Silly{
		prefixSource: new(dumb),
	}
}

func NewSillyWithPrefix(prefix animation.FrameSource) *Silly {
	return &Silly{
		prefixSource: prefix,
	}
}

func (silly *Silly) Next() bool {
	return true && silly.prefixSource.Next()
}

func (silly *Silly) Frame() string {
	prefix := silly.prefixSource.Frame()
	if prefix != "" {
		prefix = prefix + " "
	}
	return prefix + sillies[rand.Intn(len(sillies))]
}
