...
func ErrInvalidRole(params ...func(*cherry.Err)) *cherry.Err {
	err := &cherry.Err{Message: "Invalid user role", StatusHTTP: 403, ID: cherry.ErrID{SID: 0x2, Kind: 0xb}, Details: []string(nil)}
	for _, param := range params {
		param(err)
	}
	for i, detail := range err.Details {
...
func ErrAccessError(params ...func(*cherry.Err)) *cherry.Err {
	err := &cherry.Err{Message: "Access denied", StatusHTTP: 403, ID: cherry.ErrID{SID: 0x2, Kind: 0xc}, Details: []string(nil)}
	for _, param := range params {
		param(err)
	}
	for i, detail := range err.Details {
...
func ErrInternalError(params ...func(*cherry.Err)) *cherry.Err {
	err := &cherry.Err{Message: "Internal error", StatusHTTP: 500, ID: cherry.ErrID{SID: 0x2, Kind: 0xd}, Details: []string(nil)}
	for _, param := range params {
		param(err)
	}
	for i, detail := range err.Details {
...
func ErrUnableGetPodLogs(params ...func(*cherry.Err)) *cherry.Err {
	err := &cherry.Err{Message: "Unable to read logs from the pod", StatusHTTP: 500, ID: cherry.ErrID{SID: 0x2, Kind: 0xe}, Details: []string(nil)}
	for _, param := range params {
		param(err)
	}
	for i, detail := range err.Details {
